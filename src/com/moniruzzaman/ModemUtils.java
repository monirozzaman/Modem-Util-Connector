...
public class ModemUtils {
	...
	private String connect(String portName, int speed) throws Exception {
		...
		if (this.serialPort != null) {
			try {
				...
				if (type == 1) {
					String[] t5 = str.split("\r\n");
					last_cmd = t5[0];
					System.out.println("Given AT Cmd:-" + last_cmd);
				}
	...
	private class SerialReader implements SerialPortEventListener {
		...
		public void serialEvent(SerialPortEvent arg0) {
			...
			try {
				...
				//System.out.print("TwoWaySerialComm2--->199-->" + this.fileBuffer.toString());
	...
	public String ussdDial(String command, String portName) {
		try {
	...
	public List<String> getActivePorts() {
		...
		for (String port : listPorts()) {
			...
			if (!modemUtils.ussdDial("AT", port).equals("Error")) {
